...
export class InitialSchema1738276871093 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "counters" ("id" SERIAL NOT NULL, "department_id" uuid NOT NULL, "number" integer NOT NULL, "is_active" boolean NOT NULL DEFAULT true, "created_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_910bfcbadea9cde6397e0daf996" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "display_access_codes" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "department_id" uuid NOT NULL, "access_code" character varying(50) NOT NULL, "is_active" boolean NOT NULL DEFAULT true, "created_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "UQ_7697b7a4bfefe36ed04b1d40db7" UNIQUE ("access_code"), CONSTRAINT "PK_609839934378be74bada6fe649b" PRIMARY KEY ("id"))`);
        await queryRunner.query(`ALTER TABLE "counters" ADD CONSTRAINT "FK_55b46b29a10a109f7b01259709d" FOREIGN KEY ("department_id") REFERENCES "departments"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "queue_entries" ADD CONSTRAINT "FK_1321da39ce56a5c44e1c885f571" FOREIGN KEY ("department_id") REFERENCES "departments"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "queue_entries" ADD CONSTRAINT "FK_7629d57b921edf4d3e26ca5b63c" FOREIGN KEY ("counter_id") REFERENCES "counters"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE "display_access_codes" ADD CONSTRAINT "FK_a15e2410d58572abc816b1bf2df" FOREIGN KEY ("department_id") REFERENCES "departments"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
